...
const analyzeSignal = async () => {
  ...
  // Baris-baris berikut sudah dihapus dan dikomentari:
  // const midPriceLong = (targetLong + stopLossLong) / 2;
  // const midPriceShort = (targetShort + stopLossShort) / 2;
  ...
  // console.log(`   - Mid Price: ${formatPrice(midPriceLong)}`); // Dihapus dari konsol
  ...
  // console.log(`   - Mid Price: ${formatPrice(midPriceShort)}`); // Dihapus dari konsol
  ...
  return {
    ...
    longOffset,
    shortOffset,
    // midPriceLong, // Dihapus dari return
    // midPriceShort, // Dihapus dari return
    price,
    // Menambahkan kembali isCrossedUp dan isCrossedDown untuk TP Crossover
    isCrossedUp,
    isCrossedDown,
  };
};
...
*Harga Exit:* (lihat di Binance)`);
    ...
    // Monitoring internal untuk SL dan Trailing SL dari data di database
    if (db.activePosition && amtSafe !== 0) {
      ...
      if (side === "buy") {
        ...
        // Hanya memperbarui jika SL baru lebih tinggi (mengunci profit)
        if (newSL > sl) {
        ...
        const newSL = currentPrice + offset;
        // Hanya memperbarui jika SL baru lebih rendah (mengunci profit)
        if (newSL < sl) {
          db.activePosition.sl = newSL;
          saveDB();
          await sendMsg(
      ...
      // --- LOGIKA SL EKSEKUSI (TP statis dihapus) ---
      if (side === "buy") {
        // Hapus pemeriksaan TP statis (currentPrice >= tp)
        if (currentPrice <= sl) {
...
setInterval(async () => {
  try {
    ...
    // PENTING: Panggil checkPositionStatus untuk mengecek SL dan Trailing SL
    ...
    let shouldExitCurrentPosition = false; // Logika untuk swing/reversal

    if (hasBotPosition) {
      const currentSide = db.activePosition.side;
      
      // LOGIKA BARU: TP menggunakan Crossover yang berlawanan
      ...
      if (shouldTakeProfit) {
        await closePosition("TP Crossover Tercapai", db.activePosition.entryPrice);
      ...
      // LOGIKA SWING/REVERSAL (Hanya menutup posisi jika sinyal berbalik tanpa TP Crossover)
    ...
    if (db.activePosition === null && !hasActiveBinancePositionAfterClose) {
      if (sig.canLong) {
        ...
        await placeOrder(
          ...
          sig.stopLossLong,
          sig.longOffset
          // targetEntryPrice sudah dihapus
        );
    ...
    // Logika untuk memperbarui TP/SL dan offset jika ada posisi aktif
      ...
      console.log(
        "➡️ Posisi aktif terdeteksi. Memeriksa sinyal untuk pembaruan TP dan offset (SL dikelola TSL)."
      );
      if (sig.price) {
        const currentSide = db.activePosition.side;
        // newSL dihapus karena dikelola oleh TSL di checkPositionStatus
        let newTP, newOffset; 

        if (currentSide === "buy") {
          // newSL = sig.stopLossLong; // Dihapus untuk mencegah bentrokan TSL
          newTP = sig.targetLong;
          newOffset = sig.longOffset;
        } else if (currentSide === "sell") {
          // newSL = sig.stopLossShort; // Dihapus untuk mencegah bentrokan TSL
          newTP = sig.targetShort;
          newOffset = sig.shortOffset;
        }

        // Cek perubahan pada TP dan Offset saja.
        // Pembaruan SL dihapus dari kondisi 'if' dan eksekusi.
        if (
          // newSL !== db.activePosition.sl || // Dihapus
          newTP !== db.activePosition.tp ||
          newOffset !== db.activePosition.offset
        ) {
          console.log(
            `✅ Sinyal: TP/Offset baru terdeteksi! Memperbarui dari DB.`
          );
          // db.activePosition.sl = newSL; // Dihapus
          db.activePosition.tp = newTP;
          db.activePosition.offset = newOffset;
          saveDB();
        } else {
          console.log(
            "✔️ Sinyal: Tidak ada perubahan TP/Offset. Tidak ada pembaruan."
          );
        }
      } else {
        console.log(
          "⚠️ Analisis: Sinyal tidak valid. Tidak ada pembaruan TP/Offset."
        );
      }
    }
  } catch (e) {

...
test("buildSyncedActivePosition keeps existing TP/SL on scale-in when recalculate toggle is OFF", () => {
    const helpers = createExchangePositionHelpers({
        isHedgeModeEnabled: () => false,
        toFiniteNumber: (value, fallback) => {
            const numericValue = Number(value);
        ...
        getDb: () => ({
            pair: "DOGE/USDT:USDT",
            marginMode: "isolated",
            gridRecalculateExitsOnScaleIn: false,
            trailingEnabled: true,
            trailingActivateATR: 1.2,
            trailingOffsetATR: 0.6,
            gridTargetProfitUsdt: 0.5,
...
test("buildSyncedActivePosition recalculates TP/SL on scale-in when recalculate toggle is ON", () => {
    const helpers = createExchangePositionHelpers({
        isHedgeModeEnabled: () => false,
        toFiniteNumber: (value, fallback) => {
            const numericValue = Number(value);
    ...
    assert.equal(synced.targetPrice, 120);
    assert.equal(synced.stopLossPrice, 90);
});

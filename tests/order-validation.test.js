...
test("validateOrderSize does not apply PRICE_FILTER tick rules to market orders", () => {
    const result = helpers.validateOrderSize(createMarket(), 3, 2.05, { orderType: "MARKET", marketPrice: 2.05 });
    assert.equal(result.valid, true);
});

test("validateOrderSize can validate trigger prices without applying notional", () => {
    const result = helpers.validateOrderSize(createMarket(), 1, 2.0, {
...
test("validateOrderSize rejects PERCENT_PRICE_BY_SIDE violations when average price is available", () => {
    const market = {
        limits: {
            amount: { min: 1, max: 100 },
            price: { min: 0.1, max: 200 },
            cost: { min: 5, max: 500 }
        },
        averagePrice: 100,
        info: {
            filters: [
                { filterType: "PRICE_FILTER", minPrice: "0.10000000", maxPrice: "200.00000000", tickSize: "0.10000000" },
                { filterType: "LOT_SIZE", minQty: "1.00000000", maxQty: "100.00000000", stepSize: "1.00000000" },
                { filterType: "PERCENT_PRICE_BY_SIDE", bidMultiplierUp: "1.2000", bidMultiplierDown: "0.8000", askMultiplierUp: "1.1000", askMultiplierDown: "0.9000", avgPriceMins: 5 }
            ]
        }
    };

    const result = helpers.validateOrderSize(market, 1, 130, { orderType: "LIMIT", side: "buy" });
    assert.equal(result.valid, false);
    assert.match(result.reason, /PERCENT_PRICE_BY_SIDE/);
});

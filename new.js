...
const loadDB = () => {
    ...
    return {
        ...
        // NEW: Profit-based TP settings
        profitBasedTP: {
            enabled: true,
            minProfitForReversalTP: 0.5, // Minimum profit % untuk menggunakan reversal sebagai TP
            maxProfitForReversalTP: 5.0, // Maximum profit % untuk menggunakan reversal sebagai TP
            checkEntryPrice: true // Cek apakah profit sudah cukup dari entry price
        }
    };
};
...
// -------------------- PROFIT CALCULATION & TP MANAGEMENT --------------------
const calculateCurrentProfit = (entryPrice, currentPrice, side) => {
...
const shouldUseReversalAsTP = (entryPrice, currentPrice, side, reversalDirection) => {
    if (!db.profitBasedTP.enabled) return false;
    
    const currentProfit = calculateCurrentProfit(entryPrice, currentPrice, side);
    
    console.log(`💰 Profit Check: ${currentProfit.toFixed(2)}% | Min: ${db.profitBasedTP.minProfitForReversalTP}% | Max: ${db.profitBasedTP.maxProfitForReversalTP}%`);
    
    // Cek apakah profit dalam range yang diinginkan
    const isProfitInRange = currentProfit >= db.profitBasedTP.minProfitForReversalTP && 
                           currentProfit <= db.profitBasedTP.maxProfitForReversalTP;
    
    // Cek apakah reversal direction sesuai dengan posisi saat ini
    const isReversalValid = (side === 'buy' && reversalDirection === 'short') || 
                           (side === 'sell' && reversalDirection === 'long');
    
    const shouldUse = isProfitInRange && isReversalValid;
    
    if (shouldUse) {
        console.log(`🎯 REVERSAL TP ACTIVATED: Profit ${currentProfit.toFixed(2)}% dalam range, menggunakan reversal sebagai TP`);
    } else {
        console.log(`⏸️  Reversal TP skipped: Profit ${currentProfit.toFixed(2)}% di luar range atau reversal tidak valid`);
    }
...
- Profit-based TP: ${db.profitBasedTP.enabled ? `Enabled (${db.profitBasedTP.minProfitForReversalTP}-${db.profitBasedTP.maxProfitForReversalTP}%)` : 'Disabled'}`);
...
const closePosition = async (reason, entryPrice = "N/A") => {
    ...
    try {
        ...
        if (!isFinite(qty) || Math.abs(qty) === 0) {
            ...
            else if (isReversal) statusTag = "REVERSAL_TP";
...
const checkPositionStatus = async () => {
    try {
        ...
        if (db.activePosition && amtSafe !== 0) {
            ...
            // NEW: Check for reversal-based TP
            let shouldCloseForReversal = false;
            let reversalReason = "";
            
            if (side === "buy" && signalConfirmation.pendingReversal && signalConfirmation.reversalDirection === "short") {
                if (shouldUseReversalAsTP(entryPrice, currentPrice, side, signalConfirmation.reversalDirection)) {
                    shouldCloseForReversal = true;
                    reversalReason = "REVERSAL TP hit";
                }
            } else if (side === "sell" && signalConfirmation.pendingReversal && signalConfirmation.reversalDirection === "long") {
                if (shouldUseReversalAsTP(entryPrice, currentPrice, side, signalConfirmation.reversalDirection)) {
                    shouldCloseForReversal = true;
                    reversalReason = "REVERSAL TP hit";
                }
            ...
            if (side === "buy") {
                if (shouldCloseForReversal) {
                    await closePosition(reversalReason, entryPrice);
    ...
    try {
        ...
        if (db.activePosition && Math.abs(amtSafe) > MIN_POSITION_AMOUNT) {
            ...
            if (currentPrice) {
                ...
                // NEW: Calculate current profit untuk reversal TP
                const currentProfit = calculateCurrentProfit(entryPrice, currentPrice, side);
                ...
                // NEW: Profit info untuk reversal TP
                profitInfo = ` | Profit: ${currentProfit.toFixed(2)}%`;
                
                // NEW: Check reversal TP eligibility
                let reversalTPInfo = "";
                if (signalConfirmation.pendingReversal) {
                    const canUseReversalTP = shouldUseReversalAsTP(entryPrice, currentPrice, side, signalConfirmation.reversalDirection);
                    reversalTPInfo = canUseReversalTP ? " | 🎯 REVERSAL TP READY" : " | ⏸️ REVERSAL TP WAITING";
                }
                ...
                console.log("\n📊 Position Monitor");
                console.log(`   ${side.toUpperCase()} | ${status}${warning}${trailingInfo}${profitInfo}${reversalTPInfo}`);
                ...
                // NEW: Display reversal confirmation status
                if (signalConfirmation.pendingReversal) {
                    console.log(`   🔄 Reversal Confirmation: ${signalConfirmation.confirmationCount}/${signalConfirmation.requiredConfirmations} for ${signalConfirmation.reversalDirection.toUpperCase()}`);
                }
            }
        }
...
(async () => {
    ...
    console.log("🎯 NEW FEATURE: Profit-based Reversal TP Activated");
})();
...
setInterval(async () => {
    ...
    try {
        ...
        db.profitBasedTP = freshDb.profitBasedTP; // Load profit-based TP setting
    } catch (error) {
    ...
    try {
        ...
        if (Math.floor(Date.now() / 10000) % 5 === 0) {
            ...
            if (signalConfirmation.pendingReversal) {
                ...
                // NEW: Display profit info untuk reversal TP
                if (db.activePosition) {
                    const currentPrice = await getPrice();
                    if (currentPrice) {
                        const { entryPrice, side } = db.activePosition;
                        const currentProfit = calculateCurrentProfit(entryPrice, currentPrice, side);
                        const canUseReversalTP = shouldUseReversalAsTP(entryPrice, currentPrice, side, signalConfirmation.reversalDirection);
                        
                        console.log(`💰 Current Profit: ${currentProfit.toFixed(2)}% | Reversal TP: ${canUseReversalTP ? 'READY 🎯' : 'WAITING ⏸️'}`);
                    }
                }
            }
            ...
            // NEW: Display profit-based TP status
            console.log(`🎯 Profit-based TP: ${db.profitBasedTP.enabled ? `Aktif (${db.profitBasedTP.minProfitForReversalTP}-${db.profitBasedTP.maxProfitForReversalTP}%)` : 'Nonaktif'}`);
        }
    } catch (err) {

...
const { SMA, RSI, EMA, MACD, Stochastic, OBV } = require("technicalindicators");
...
const TIMEFRAMES = {
    FAST: "3m",      // Untuk entry cepat
    MEDIUM: "5m",    // Untuk konfirmasi trend
    SLOW: "15m"      // Untuk trend utama
};
...
// -------------------- MOMENTUM ANALYSIS --------------------
const updateMomentumTracker = (signals) => {
    const now = Date.now();
    ...
    if (bullishSignals > bearishSignals) {
        momentumTracker.trend = 'bullish';
        momentumTracker.strength = totalStrength / signals.length;
    } else if (bearishSignals > bullishSignals) {
        momentumTracker.trend = 'bearish';
        momentumTracker.strength = totalStrength / signals.length;
    } else {
        momentumTracker.trend = 'neutral';
        momentumTracker.strength = 0;
    }

    momentumTracker.lastUpdate = now;
    momentumTracker.confirmationSignals = signals;

    console.log(`📊 Momentum Update: ${momentumTracker.trend.toUpperCase()} | Strength: ${momentumTracker.strength.toFixed(2)}`);
};
...
- Advanced Signals: Multi-timeframe: ${db.advancedSignals.multiTimeframe}, Momentum: ${db.advancedSignals.momentumConfirmation}, Volume: ${db.advancedSignals.volumeAnalysis}`);
...
const analyzeMultiTimeframe = async () => {
    ...
    try {
        const timeframes = [
            { tf: TIMEFRAMES.FAST, weight: 0.3, name: "FAST(3m)" },
            { tf: TIMEFRAMES.MEDIUM, weight: 0.5, name: "MEDIUM(5m)" },
            { tf: TIMEFRAMES.SLOW, weight: 0.2, name: "SLOW(15m)" }
        ];
        ...
        for (const timeframe of timeframes) {
            try {
                const ohlcv = await safeApiCall(exchange.fetchOHLCV, db.pair, timeframe.tf, undefined, 100);
                if (!ohlcv || ohlcv.length < 50) {
                    console.warn(`⚠️ Insufficient data for ${timeframe.name}`);
                    continue;
                }
                ...
                if (!currentPrice) currentPrice = close[close.length - 1];

                // Calculate indicators
                const maFast = SMA.calculate({ values: close, period: 8 });
                const maMedium = SMA.calculate({ values: close, period: 21 });
                const maSlow = SMA.calculate({ values: close, period: 50 });
                ...
                const maBullish = currentMAFast > currentMAMedium && currentMAMedium > currentMASlow;
                const maBearish = currentMAFast < currentMAMedium && currentMAMedium < currentMASlow;
                
                if (maBullish) {
                    strength += 0.4;
                    signalComponents.push("MA_BULLISH");
                } else if (maBearish) {
                    strength -= 0.4;
                    signalComponents.push("MA_BEARISH");
                }

                // RSI Momentum (20% weight)
                if (currentRSI > 60 && currentRSI < 80) {
                    strength += 0.2;
                    signalComponents.push("RSI_BULLISH");
                } else if (currentRSI < 40 && currentRSI > 20) {
                    strength -= 0.2;
                    signalComponents.push("RSI_BEARISH");
                }

                // MACD Momentum (20% weight)
                if (currentMACD && currentMACD.histogram > 0) {
                    strength += 0.2;
                    signalComponents.push("MACD_BULLISH");
                } else if (currentMACD && currentMACD.histogram < 0) {
                    strength -= 0.2;
                    signalComponents.push("MACD_BEARISH");
                }
                ...
                if (volumeRatio > 1.2) {
                    ...
                    if (strength > 0) {
                        strength += 0.1;
                        signalComponents.push("VOLUME_CONFIRM");
                    } else if (strength < 0) {
                        strength -= 0.1;
                        signalComponents.push("VOLUME_CONFIRM");
                    }
                ...
                if (currentStoch && currentStoch.k > 80) {
                    strength -= 0.1; // Overbought
                    signalComponents.push("STOCH_OVERBOUGHT");
                } else if (currentStoch && currentStoch.k < 20) {
                    strength += 0.1; // Oversold
                    signalComponents.push("STOCH_OVERSOLD");
                }
                ...
                allSignals.push(timeframeSignal);

                console.log(`📊 ${timeframe.name} Analysis: ${trend.toUpperCase()} (Strength: ${strength.toFixed(2)}) - ${signalComponents.join(', ')}`);

            } catch (error) {
                console.error(`❌ Error in ${timeframe.name} analysis:`, error.message);
            }
        }
        ...
        allSignals.forEach(signal => {
            ...
            combinedStrength += weightedStrength;
        ...
        // Update momentum tracker
        updateMomentumTracker(allSignals);

        console.log(`🎯 Combined Signal Strength: ${combinedStrength.toFixed(3)}`);
...
const analyzeEnhancedSignal = async () => {
    ...
    try {
        // Multi-timeframe analysis
        const multiTFResult = await analyzeMultiTimeframe();
        
        if (!multiTFResult.price) {
            console.warn("⚠️ No price data from multi-timeframe analysis");
            return {};
        }

        const price = multiTFResult.price;
        const combinedStrength = multiTFResult.combinedStrength;
        const minStrength = db.advancedSignals.minSignalStrength;

        // Get detailed 5m data for S/R levels
        const ohlcv = await safeApiCall(exchange.fetchOHLCV, db.pair, "5m", undefined, 200);
        if (!ohlcv || ohlcv.length < 100) {
            console.warn("⚠️ Insufficient OHLCV data for S/R analysis");
            return {};
        }
        ...
        // Determine signals based on multi-timeframe analysis
        let canLong = false;
        let canShort = false;
...
📊 Combined Strength: ${combinedStrength.toFixed(3)}
📊 Momentum Trend: ${momentumTracker.trend.toUpperCase()}
...
══════════════════════════════════════════════════`);
        ...
        console.log("🕒 Multi-Timeframe Signals:");
        multiTFResult.signals.forEach(signal => {
            const emoji = signal.trend === 'bullish' ? '📈' : signal.trend === 'bearish' ? '📉' : '➡️';
            console.log(`   ${emoji} ${signal.timeframe}: ${signal.trend.toUpperCase()} (${signal.weightedStrength.toFixed(3)})`);
        });
...
(async () => {
    ...
    console.log("\n🚀 Bot started with ENHANCED MULTI-TIMEFRAME ANALYSIS, MOMENTUM CONFIRMATION and ADVANCED SIGNAL DETECTION");
})();
...
setInterval(async () => {
    ...
    try {
        ...
        db.advancedSignals = freshDb.advancedSignals;
    } catch (error) {
    ...
    try {
        ...
        if (db.activePosition === null && !hasActiveBinancePositionAfterClose) {
            if (signal.canLong) {
                ...
                if (!isLongBreakout) {
                    console.log(`🚀 LONG Signal | Strength: ${signal.signalStrength.toFixed(3)} | TP: ${formatPrice(signal.targetLong)} | SL: ${formatPrice(signal.stopLossLong)}`);
                ...
                if (!isShortBreakout) {
                    console.log(`📉 SHORT Signal | Strength: ${signal.signalStrength.toFixed(3)} | TP: ${formatPrice(signal.targetShort)} | SL: ${formatPrice(signal.stopLossShort)}`);
        ...
        if (Math.floor(Date.now() / 10000) % 5 === 0) {
            ...
            console.log(`📈 Momentum Status: ${momentumTracker.trend.toUpperCase()} (Strength: ${momentumTracker.strength.toFixed(2)})`);
